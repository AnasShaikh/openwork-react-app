...
import {NativeAthena, IOpenworkGenesis} from "../src/suites/current-mainnet/native/native-athena-v5.sol";

contract MockAthenaV5Genesis {
...
contract MockAthenaV5ActivityTracker {
    function oracleActiveStatus(string memory) external pure returns (bool) {
        return true;
    }
}

contract CurrentMainnetNativeAthenaV5Test is Test {
    NativeAthena internal athena;
    MockAthenaV5Genesis internal genesis;
    ...
    function setUp() public {
        genesis = new MockAthenaV5Genesis();
        genesis.setJob(giver, applicant, IOpenworkGenesis.JobStatus.InProgress);

        address athenaAddress = makeAddr("athena-v5");
        vm.etch(
            athenaAddress, vm.getDeployedCode("src/suites/current-mainnet/native/native-athena-v5.sol:NativeAthena")
        );
        athena = NativeAthena(payable(athenaAddress));
        athena.initialize(address(this), address(0), address(genesis), address(0), address(0));
        athena.setActivityTracker(address(new MockAthenaV5ActivityTracker()));
