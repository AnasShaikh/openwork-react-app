...
interface IOpenworkGenesis {
    ...
    function setUserCumulativeEarnings(address user, uint256 earnings) external;
...
contract NativeOpenWorkJobContract is Ownable
{
    ...
    constructor(address /* _bridge */, address _genesis) Ownable(msg.sender) {
    ...
    function _accumulateJobTokens(address user, uint256 amountUSDT) private {
        (uint256 currentCumulative, ) = genesis.getUserRewardInfo(user);
        ...
        // Update user's cumulative earnings and total tokens in genesis
        genesis.setUserCumulativeEarnings(user, newCumulative);
        ...
        emit TokensEarned(user, tokensToAward, newCumulative, currentTotalTokens + tokensToAward);
    }
