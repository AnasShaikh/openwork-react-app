...
function sendUpgradeCommand(
    ...
    require(msg.value > 0, "No fee provided for cross-chain message");
    
    // Validate destination chain is configured
    require(_hasPeerForChain(_dstChainId), "No peer configured for destination chain");
    
    // Create upgrade payload with enhanced structure
    bytes memory upgradePayload = _createUpgradePayload(targetProxy, newImplementation);
    
    // Calculate required fee upfront
    ...
    // Execute LayerZero send with enhanced error handling
    try this.executeLzSend(_dstChainId, upgradePayload, _options, requiredFee) {
        ...
        revert("Upgrade send failed with unknown error");
    }
}

// External function for protected LayerZero send (allows try/catch)
function executeLzSend(
    uint32 _dstChainId,
    ...
    _lzSend(
        ...
        _options,
        _fee,
        payable(tx.origin)  // Refund to transaction originator
    );
}

// Create structured upgrade payload
function _createUpgradePayload(
    address targetProxy,
    address newImplementation
) internal pure returns (bytes memory) {
    // Use exact same encoding as working governance messages
    return abi.encode(
        "upgradeFromDAO",           // Function identifier
        targetProxy,                // Target contract to upgrade
        newImplementation           // New implementation address
    );
}

// Check if peer is configured for destination chain
function _hasPeerForChain(uint32 _dstChainId) internal view returns (bool) {
    ...
    function quoteUpgradeCommand(
        ...
        // Use same payload structure as actual send
        bytes memory payload = _createUpgradePayload(targetProxy, newImplementation);
