    ...
    UUPSUpgradeable
{
    ...
    // ==================== FIXED VOTING FUNCTIONS - NO SILENT FAILURES ====================
    ...
    function _voteOnDispute(
        ...
        // FIXED: NO SILENT FAILURES - Let transactions revert on errors
        require(address(bridge) != address(0), "Bridge not set");
        ...
        // Split msg.value equally between both chains
        uint256 halfValue = msg.value / 2;
        require(halfValue > 0, "Insufficient fee provided");
        
        // Send to rewards chain - WILL REVERT ON FAILURE
        bridge.sendToRewardsChain{value: halfValue}("notifyGovernanceAction", rewardsPayload, _rewardsOptions);
        
        // Send to athena client chain - WILL REVERT ON FAILURE  
        bridge.sendToAthenaClientChain{value: halfValue}("recordVote", athenaClientPayload, _athenaClientOptions);
    }
    ...
    // FIXED: Function to finalize dispute - also no silent failures
