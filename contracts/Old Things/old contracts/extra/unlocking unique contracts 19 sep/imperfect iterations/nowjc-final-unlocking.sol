    ...
    UUPSUpgradeable
{
    ...
    function setCCTPTransceiver(address _cctpTransceiver) external onlyOwner {
        require(_cctpTransceiver != address(0), "CCTP transceiver address cannot be zero");
    ...
    function releasePaymentCrossChain(
        ...
        // Withdraw funds from CCTP receiver to this contract
        (bool withdrawSuccess, ) = cctpReceiver.call(
            abi.encodeWithSignature("withdrawFunds(address,uint256)", address(this), _amount)
        );
        require(withdrawSuccess, "CCTP receiver withdrawal failed");
        
        // Approve CCTP transceiver to spend tokens
        ...
        (bool sendSuccess, ) = cctpTransceiver.call(
        ...
        require(sendSuccess, "CCTP cross-chain transfer failed");
        ...
        emit PaymentReleased(_jobId, _jobGiver, _targetRecipient, _amount, job.currentMilestone);
    }
