    ...
    UUPSUpgradeable
{
    ...
    event CrossChainPaymentReleased(string indexed jobId, address indexed recipient, uint256 amount, uint32 targetDomain);
    ...
    function handleReleasePaymentCrossChain(
        ...
        // Note: This handler is kept for compatibility but direct calls are preferred
        // The main releasePaymentCrossChain function is defined later in the contract
        require(false, "Use direct releasePaymentCrossChain function");
    }
    ...
    /**
     * @dev Release payment directly to job applicant on target chain
     * This is the NEW implementation that sends USDC directly to the job applicant's wallet
     * rather than to the target chain LOWJC contract
     */
    function releasePaymentCrossChain(
        string memory _jobId,
        uint32 _targetChainDomain,
        address _targetRecipient,
        bytes memory _crossChainData
    ) external payable {
        IOpenworkGenesis.Job memory job = genesis.getJob(_jobId);
        ...
        require(_targetChainDomain > 0, "Invalid domain");
        
        // ==================== SMART MILESTONE AMOUNT RESOLUTION ====================
        
        uint256 _amount;
        
        // Check if finalMilestones array exists and has elements
        if (job.finalMilestones.length > 0) {
            // Ensure currentMilestone is valid (1-based indexing)
            require(job.currentMilestone > 0 && job.currentMilestone <= job.finalMilestones.length, "Invalid milestone");
            
            // Get amount from current milestone (convert from 1-based to 0-based indexing)
            _amount = job.finalMilestones[job.currentMilestone - 1].amount;
        } else if (job.milestonePayments.length > 0) {
            // Fallback to original milestonePayments if finalMilestones is empty
            require(job.currentMilestone > 0 && job.currentMilestone <= job.milestonePayments.length, "Invalid milestone");
            
            // Get amount from current milestone (convert from 1-based to 0-based indexing)
            _amount = job.milestonePayments[job.currentMilestone - 1].amount;
        } else {
            revert("No milestones found for job");
        }
        
        require(_amount > 0, "Invalid milestone amount");
        
        // Verify this contract has sufficient USDC balance
        require(usdtToken.balanceOf(address(this)) >= _amount, "Insufficient contract balance");
        
        // ==================== DIRECT PAYMENT TO APPLICANT ====================
        
        // ✅ KEY CHANGE: Send directly to job applicant, not to target chain LOWJC
        ...
        // Send USDC via CCTP directly to job applicant on target chain
        ICCTPTransceiver(cctpTransceiver).sendFast(
            ...
            mintRecipient,       // ✅ Direct to job applicant wallet
        ...
        emit CrossChainPaymentReleased(_jobId, _targetRecipient, _amount, _targetChainDomain);
        emit PaymentReleased(_jobId, job.jobGiver, _targetRecipient, _amount, job.currentMilestone);
    }
