...
interface INativeOpenWorkJobContract {
    ...
    function postJob(string memory jobId, address jobGiver, string memory jobDetailHash, string[] memory descriptions, uint256[] memory amounts, uint32 posterChainDomain, address posterAddress) external;
...
contract NativeChainBridge is OAppSender, OAppReceiver {
    ...
    function _lzReceive(
        ...
        // ==================== NATIVE OPENWORK JOB CONTRACT MESSAGES ====================
            ...
            require(nativeOpenWorkJobContract != address(0), "Native OpenWork Job contract not set");
            (, string memory jobId, address jobGiver, string memory jobDetailHash, string[] memory descriptions, uint256[] memory amounts, uint32 posterChainDomain, address posterAddress) = abi.decode(_message, (string, string, address, string, string[], uint256[], uint32, address));
            INativeOpenWorkJobContract(nativeOpenWorkJobContract).postJob(jobId, jobGiver, jobDetailHash, descriptions, amounts, posterChainDomain, posterAddress);
