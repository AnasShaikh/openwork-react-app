...
// Interface for DisputeSettlementManager
interface IDisputeSettlementManager {
...
contract NativeAthenaWithDisputeSettlementManager is 
    Initializable,
    OwnableUpgradeable,
    UUPSUpgradeable
{
    ...
    // ADDED: DisputeSettlementManager for complete dispute resolution
    IDisputeSettlementManager public disputeSettlementManager;
    ...
    /**
     * @notice UPDATED: Uses DisputeSettlementManager for complete dispute settlement
     * @dev DisputeSettlementManager handles both fee distribution and disputed funds
     */
    function processFeePayment(
        ...
        if (address(disputeSettlementManager) != address(0)) {
            // Approve DisputeSettlementManager to spend USDC for fee distribution
            IERC20(usdcToken).approve(address(disputeSettlementManager), _totalFees);
            
            // Let DisputeSettlementManager handle everything:
            // - Fee distribution to winning voters
            // - Disputed funds transfer via NOWJC
    ...
    function setDisputeSettlementManager(address _disputeSettlementManager) external onlyOwner {
        ...
        disputeSettlementManager = IDisputeSettlementManager(_disputeSettlementManager);
