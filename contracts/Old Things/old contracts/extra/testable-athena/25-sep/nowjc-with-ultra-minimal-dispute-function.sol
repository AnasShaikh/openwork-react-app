...
contract NOWJCWithUltraMinimalDisputeFunction is 
    Initializable,
    OwnableUpgradeable,
    UUPSUpgradeable
{
    ...
    // ADDED: Dispute Settlement Manager address (replaces Native Athena)
    address public disputeSettlementManager;
    ...
    event DisputedFundsReleased(string indexed disputeType, address indexed winner, uint32 indexed chainDomain, uint256 amount);
    event DisputeSettlementManagerSet(address indexed disputeSettlementManager);
    ...
    function setDisputeSettlementManager(address _disputeSettlementManager) external onlyOwner {
        require(_disputeSettlementManager != address(0), "Invalid address");
        disputeSettlementManager = _disputeSettlementManager;
        emit DisputeSettlementManagerSet(_disputeSettlementManager);
    }
    
    // ==================== ULTRA-MINIMAL DISPUTE RESOLUTION ====================
    
    /**
     * @notice Ultra-minimal disputed funds release - only does CCTP transfer
     * @dev All complex logic handled by DisputeSettlementManager
     * @param _amount Amount of USDC to send cross-chain (already calculated by DSM)
     * @param _winner Winner address on target chain (already determined by DSM)  
     * @param _winnerChainDomain CCTP domain for target chain (already parsed by DSM)
     */
    function releaseDisputedFunds(uint256 _amount, address _winner, uint32 _winnerChainDomain) external {
        ...
        // Ultra-minimal: Just do CCTP transfer using existing working pattern
        usdtToken.approve(cctpTransceiver, _amount);
        ICCTPTransceiver(cctpTransceiver).sendFast(
            _amount,
            _winnerChainDomain, 
            bytes32(uint256(uint160(_winner))),
            1000
        );
