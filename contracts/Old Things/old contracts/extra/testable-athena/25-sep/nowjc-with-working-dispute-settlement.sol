...
contract NOWJCWithWorkingDisputeSettlement is 
    Initializable,
    OwnableUpgradeable,
    UUPSUpgradeable
{
    ...
    // Native Athena address for dispute settlement authorization
    ...
    event DisputedFundsReleased(string indexed jobId, address indexed winner, uint32 indexed chainDomain, uint256 amount);
    ...
    modifier onlyNativeAthena() {
        require(msg.sender == nativeAthena, "Only Native Athena can call this function");
        _;
    }
    ...
    // ==================== NEW: DISPUTE SETTLEMENT FUNCTIONS ====================
    
    /**
     * @notice Release disputed funds cross-chain to dispute winner
     * @dev Called by Native Athena after dispute resolution
     * @param _jobId Job/dispute ID
     * @param _winner Address of dispute winner on target chain
     * @param _winnerChainDomain CCTP domain for target chain
     * @param _amount Amount of USDC to send cross-chain
     */
