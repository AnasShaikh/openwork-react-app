    ...
    UUPSUpgradeable
{
    ...
    // Simple function to test CCTP fix - just send some funds cross-chain
    function testCCTPFix(address _recipient, uint256 _amount, uint32 _targetChain) external {
        require(_amount > 0, "Amount must be greater than 0");
        require(cctpTransceiver != address(0), "CCTP Transceiver not set");
        
        // Use our FIXED CCTP pattern (approve → sendFast)
        usdtToken.approve(cctpTransceiver, _amount);
        ICCTPTransceiver(cctpTransceiver).sendFast(
            _amount,
            _targetChain,
        ...
        emit DisputedFundsReleased("test", _recipient, _targetChain, _amount);
    }
}