    ...
    UUPSUpgradeable
{
    ...
    // OLD FUNCTION REMOVED - Now using modular approach with 3 separate functions
    
    // OLD INTERNAL FUNCTION REMOVED - Logic moved to modular functions
    ...
    // ==================== MODULAR DISPUTE SETTLEMENT - 3 INDEPENDENT FUNCTIONS ====================
    
    /**
     * @dev Step 1: Calculate votes and store winning side in Genesis
     * @param _disputeId The dispute to finalize
     * @return winningSide True if dispute winner (votesFor > votesAgainst)
     * @return disputeWinner Address of the dispute winner (job giver or applicant)
     * @return disputedAmount Amount being disputed
     * @return winnerChainDomain Chain domain where winner should receive funds
     */
    function calculateVotesAndStoreResults(string memory _disputeId) external returns (
        ...
        // Mark dispute as finalized in Genesis
        genesis.finalizeDispute(_disputeId, winningSide);
        
        // Determine dispute winner and amount
        if (address(nowjContract) != address(0) && nowjContract.jobExists(_disputeId)) {
            ...
            // Get disputed amount from Genesis
            disputedAmount = dispute.disputedAmount;
            
            if (winningSide) {
                // Job giver wins dispute
                disputeWinner = jobGiver;
                winnerChainDomain = 3; // Default to native chain (Arbitrum)
            } else {
                // Applicant wins dispute
                disputeWinner = selectedApplicant;
                winnerChainDomain = nowjContract.jobApplicantChainDomain(_disputeId, selectedApplicant);
        ...
        emit DisputedFundsResolved(_disputeId, disputeWinner, winningSide);
        
        return (winningSide, disputeWinner, disputedAmount, winnerChainDomain);
    }
    
    /**
     * @dev Step 2: Pay disputed funds to the winner
     * @param _disputeId The dispute ID
     * @param _winner Address to receive disputed funds
     * @param _amount Amount to transfer
     * @param _targetChainDomain Chain domain for transfer
     */
    function payDisputeWinner(
        ...
        // Validate dispute is finalized
        IOpenworkGenesis.Dispute memory dispute = genesis.getDispute(_disputeId);
        require(dispute.isFinalized, "Dispute not finalized");
        require(_winner != address(0), "Invalid winner");
        require(_amount > 0, "Invalid amount");
        
        // Call NOWJC to release disputed funds
        if (address(nowjContract) != address(0)) {
            nowjContract.releaseDisputedFunds(_winner, _amount, _targetChainDomain);
        }
        
        emit DisputedFundsResolved(_disputeId, _winner, dispute.result);
    }
    
    /**
     * @dev Step 3: Pay fees to winning voters
     * @param _disputeId The dispute ID
     */
    function payVoterFees(string memory _disputeId) external {
        ...
        IOpenworkGenesis.Dispute memory dispute = genesis.getDispute(_disputeId);
        require(dispute.isFinalized, "Dispute not finalized");
        require(dispute.fees > 0, "No fees to distribute");
        require(dispute.fees <= accumulatedFees, "Insufficient accumulated fees");
        
        // Get all voter data
        IOpenworkGenesis.VoterData[] memory allVoterData = genesis.getDisputeVoters(_disputeId);
        require(allVoterData.length > 0, "No voters found");
        
        // Count winning voters and calculate total winning voting power
        uint256 winningVoterCount = 0;
        uint256 totalWinningVotingPower = 0;
        bool winningSide = dispute.result;
        
        for (uint256 i = 0; i < allVoterData.length; i++) {
            if (allVoterData[i].voteFor == winningSide) {
                winningVoterCount++;
                totalWinningVotingPower += allVoterData[i].votingPower;
            }
        }
        
        require(totalWinningVotingPower > 0, "No winning voting power");
        
        // Distribute fees to winning voters
        for (uint256 i = 0; i < allVoterData.length; i++) {
            if (allVoterData[i].voteFor == winningSide) {
                uint256 voterShare = (allVoterData[i].votingPower * dispute.fees) / totalWinningVotingPower;
                
                if (voterShare > 0) {
                    usdcToken.safeTransfer(allVoterData[i].claimAddress, voterShare);
                    accumulatedFees -= voterShare;
                    emit FeePaymentProcessed(_disputeId, allVoterData[i].claimAddress, voterShare);
                }
            }
        }
    }
