    ...
    UUPSUpgradeable
{
    ...
    /**
     * @dev Release disputed job funds to the dispute winner (Binary: current milestone amount)
     * @param _jobId The job ID with disputed funds
     * @param _winner The address of the dispute winner (job giver or applicant)
     */
    function releaseDisputedFunds(
        ...
        require(currentMilestoneAmount > 0, "No funds to dispute");
        
        // Determine winner's preferred chain domain
        uint32 winnerChainDomain;
        if (_winner == job.selectedApplicant) {
            // Applicant wins - use their stored preferred chain domain
            winnerChainDomain = jobApplicantChainDomain[_jobId][_winner];
            require(winnerChainDomain > 0, "Applicant chain domain not found");
        } else {
            // Job giver wins - default to native chain (Arbitrum = 3)
            // TODO: In future, job giver could also specify preferred domain when posting job
            winnerChainDomain = 3;
        }
        ...
        if (winnerChainDomain == 3) {
            ...
            usdtToken.approve(cctpTransceiver, currentMilestoneAmount);
            ICCTPTransceiver(cctpTransceiver).sendFast(
        ...
        // Mark job as cancelled to prevent further payments
        genesis.updateJobStatus(_jobId, IOpenworkGenesis.JobStatus.Cancelled);
        
        emit DisputedFundsReleased(_jobId, _winner, winnerChainDomain, currentMilestoneAmount);
    }
}