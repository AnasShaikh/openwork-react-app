...
interface IOpenworkGenesis {
    ...
    // NEW: Payment target setter functions
    ...
    UUPSUpgradeable
{
    ...
    // NEW: CCTP Integration for cross-chain payment release
    ...
    uint32 public currentChainCCTPDomain; // Current chain's CCTP domain (Arbitrum Sepolia = 3)
    ...
    function setMultipleChainCCTPReceivers(uint32[] memory _domains, address[] memory _receivers) external onlyOwner {
        require(_domains.length == _receivers.length, "Arrays length mismatch");
        for (uint i = 0; i < _domains.length; i++) {
            require(_domains[i] > 0, "Invalid CCTP domain");
    ...
    // NEW: Cross-Chain Payment Release Function
    ...
    function applyToJob(
        ...
        genesis.setJobApplication(
            ...
            _descriptions, 
            _amounts,
            _preferredPaymentChainDomain,    // NEW: Pass payment preferences
            _preferredPaymentAddress         // NEW: Pass payment preferences
        );
    ...
    function startJob(
        ...
        uint32 _posterChainDomain,           // NEW PARAMETER
        address _posterAddress,              // NEW PARAMETER
        uint32 _paymentTargetChainDomain,    // NEW PARAMETER
        address _paymentTargetAddress        // NEW PARAMETER
    ) external {
        ...
        // NEW: Store payment target information  
        // Use current chain as applier origin (since this is where the message came from)
    ...
    function releasePayment(
        ...
        uint32 _paymentTargetChainDomain,  // NEW: Where to send payment
        address _paymentTargetAddress      // NEW: Who receives payment
    ) external {
        require(msg.sender == bridge, "Only bridge can call this function");
        ...
        // NEW: Use cross-chain payment release instead of withdrawFunds
        // Default to applier's original address if no preference set
