    ...
    Ownable
{
    ...
    function isValidPaymentDomain(uint32 _domain) public view returns (bool) {
        // Domain is valid if it's the current chain or the native chain
        // In production, this should check bridge peer registry
        return _domain == chainId || _domain == nativeChainDomain || _domain > 0;
    }
  ...
  function createProfile(
    ...
    function applyToJob(
        ...
        // NEW: Validate payment chain domain and address
        if (_preferredPaymentChainDomain != 0) {
            require(isValidPaymentDomain(_preferredPaymentChainDomain), "Invalid payment chain domain");
            require(_preferredPaymentAddress != address(0), "Invalid payment address");
        }
        ...
        // NEW: Store payment preferences (default to current chain if not specified)
        newApp.preferredPaymentChainDomain = _preferredPaymentChainDomain != 0 ? _preferredPaymentChainDomain : chainId;
        newApp.preferredPaymentAddress = _preferredPaymentAddress != address(0) ? _preferredPaymentAddress : msg.sender;
        ...
        // Send to native chain with payment preferences
        bytes memory payload = abi.encode(
            "applyToJob", 
            msg.sender, 
            _jobId, 
            ...
            _descriptions, 
            _amounts,
            newApp.preferredPaymentChainDomain,  // NEW
            newApp.preferredPaymentAddress       // NEW
        );
    ...
    function startJob(
        ...
        job.paymentTargetChainDomain = app.preferredPaymentChainDomain;
        job.paymentTargetAddress = app.preferredPaymentAddress;
        job.applierOriginChainDomain = chainId;
        ...
        // Send to native chain with payment target info
        bytes memory payload = abi.encode(
            "startJob", 
            msg.sender, 
            _jobId, 
    ...
    function releasePayment(
        ...
        // Send to native chain with payment target info for cross-chain release
