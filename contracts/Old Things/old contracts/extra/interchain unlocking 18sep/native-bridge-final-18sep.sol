...
interface INativeOpenWorkJobContract {
    ...
    function applyToJob(address applicant, string memory jobId, string memory applicationHash, string[] memory descriptions, uint256[] memory amounts, uint32 preferredPaymentChainDomain, address preferredPaymentAddress) external;
    function startJob(address jobGiver, string memory jobId, uint256 applicationId, bool useApplicantMilestones, uint32 posterChainDomain, address posterAddress, uint32 paymentTargetChainDomain, address paymentTargetAddress) external;
    function submitWork(address applicant, string memory jobId, string memory submissionHash) external;
    function releasePayment(address jobGiver, string memory jobId, uint256 amount, address applierAddress, uint32 applierChainDomain, uint256 milestone, uint32 posterChainDomain, address posterAddress, uint32 paymentTargetChainDomain, address paymentTargetAddress) external;
...
contract NativeChainBridge is OAppSender, OAppReceiver {
    ...
    function _lzReceive(
        ...
        // ==================== NATIVE OPENWORK JOB CONTRACT MESSAGES ====================
            ...
            require(nativeOpenWorkJobContract != address(0), "Native OpenWork Job contract not set");
            (, address applicant, string memory jobId, string memory applicationHash, string[] memory descriptions, uint256[] memory amounts, uint32 preferredPaymentChainDomain, address preferredPaymentAddress) = abi.decode(_message, (string, address, string, string, string[], uint256[], uint32, address));
            INativeOpenWorkJobContract(nativeOpenWorkJobContract).applyToJob(applicant, jobId, applicationHash, descriptions, amounts, preferredPaymentChainDomain, preferredPaymentAddress);
        } else if (keccak256(bytes(functionName)) == keccak256(bytes("startJob"))) {
            require(nativeOpenWorkJobContract != address(0), "Native OpenWork Job contract not set");
            (, address jobGiver, string memory jobId, uint256 applicationId, bool useApplicantMilestones, uint32 posterChainDomain, address posterAddress, uint32 paymentTargetChainDomain, address paymentTargetAddress) = abi.decode(_message, (string, address, string, uint256, bool, uint32, address, uint32, address));
            INativeOpenWorkJobContract(nativeOpenWorkJobContract).startJob(jobGiver, jobId, applicationId, useApplicantMilestones, posterChainDomain, posterAddress, paymentTargetChainDomain, paymentTargetAddress);
            ...
            require(nativeOpenWorkJobContract != address(0), "Native OpenWork Job contract not set");
            (, address jobGiver, string memory jobId, uint256 amount, address applierAddress, uint32 applierChainDomain, uint256 milestone, uint32 posterChainDomain, address posterAddress, uint32 paymentTargetChainDomain, address paymentTargetAddress) = abi.decode(_message, (string, address, string, uint256, address, uint32, uint256, uint32, address, uint32, address));
            INativeOpenWorkJobContract(nativeOpenWorkJobContract).releasePayment(jobGiver, jobId, amount, applierAddress, applierChainDomain, milestone, posterChainDomain, posterAddress, paymentTargetChainDomain, paymentTargetAddress);
