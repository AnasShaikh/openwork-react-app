 ...
 * @notice Fixed for genesis deployment - uses immutable owner and fallback logic
 */
contract OpenworkGenesis {
    ...
    // Access control - Fixed for genesis deployment
    mapping(address => bool) public authorizedContracts;
    address public owner;
    
    // Immutable fallback owner for genesis deployment
    address private immutable GENESIS_OWNER = 0xfD08836eeE6242092a9c869237a8d122275b024A;
    ...
    modifier onlyOwner() {
        require(msg.sender == getEffectiveOwner(), "Not owner");
        _;
    }
    
    modifier onlyAuthorized() {
        require(authorizedContracts[msg.sender] || msg.sender == getEffectiveOwner(), "Not authorized");
        _;
    }
    ...
    constructor() {
        // Don't set owner here - will be handled by getEffectiveOwner()
        // This allows the contract to work both in genesis and regular deployment
    }

    // ==================== ACCESS CONTROL ====================
    
    /**
     * @dev Returns the effective owner - falls back to GENESIS_OWNER if owner is not set
     * This ensures the contract works when deployed in genesis (constructor doesn't run)
     */
    function getEffectiveOwner() public view returns (address) {
        return owner == address(0) ? GENESIS_OWNER : owner;
    }
    
    /**
     * @dev Initialize the contract - can only be called once and only by genesis owner
     * This replaces the constructor for genesis deployment
     */
    function initialize() external {
        require(owner == address(0), "Already initialized");
        require(msg.sender == GENESIS_OWNER, "Only genesis owner can initialize");
        owner = GENESIS_OWNER;
        emit OwnershipTransferred(address(0), GENESIS_OWNER);
    }
    
    function transferOwnership(address newOwner) external onlyOwner {
        require(newOwner != address(0), "Zero address");
        address oldOwner = getEffectiveOwner();
        owner = newOwner;
