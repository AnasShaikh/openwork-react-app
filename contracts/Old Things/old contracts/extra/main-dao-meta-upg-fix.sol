    ...
    UUPSUpgradeable
{
    ...
    // Chain EIDs for upgrade routing
    uint32 public nativeChainEid;   // Native chain (Arbitrum Sepolia: 40231)
    uint32 public localChainEid;    // Local chain (OP Sepolia: 40232)
    ...
    function setLocalChainEid(uint32 _localChainEid) external onlyOwner {
    ...
    function upgradeContract(
        ...
        if (lz_eid == chainId) {
            ...
            // Create upgrade payload in Main DAO
            bytes memory upgradePayload = abi.encode(
                "upgradeFromDAO",
                targetProxy,
                newImplementation
            );
            
            // Route to correct chain based on LZ EID
            if (lz_eid == nativeChainEid) {  // Native chain 
                bridge.sendToNativeChain{value: msg.value}(
                    "upgradeFromDAO",
                    upgradePayload,
                    _options
                );
            } else if (lz_eid == localChainEid) {  // Local/LOWJC chain
                bridge.sendToLowjcChain{value: msg.value}(
                    "upgradeFromDAO", 
                    upgradePayload,
                    _options
                );
