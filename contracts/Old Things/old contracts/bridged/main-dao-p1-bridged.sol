...
contract MainDAO is Governor, GovernorSettings, GovernorCountingSimple, Ownable {
    ...
    uint256 public quorumAmount = 50 * 10**18;
    ...
    event CrossChainBridgeUpdated(address indexed newBridge);
        ...
        Ownable(_owner)
    {
        ...
        // Create an internal instance of RewardsCalculator
        rewardsCalculator = new RewardsCalculator(_owner);
    }
    
    // Set cross-chain bridge contract
    function setCrossChainBridge(address _bridge) external onlyOwner {
        crossChainBridge = ICrossChainBridge(_bridge);
        emit CrossChainBridgeUpdated(_bridge);
    }
    ...
    function _sendStakeDataCrossChain(address staker, bool isActive) internal {
        if (address(crossChainBridge) != address(0)) {
            try crossChainBridge.sendStakeUpdate(staker, isActive) {
                // Success - could emit event if needed
            } catch {
                // Failed - could emit event if needed
            }
        }
    }
    ...
    function calculateTotalRewards(address user) public view returns (uint256) {
        ...
        // Use internal RewardsCalculator instance
    ...
    function updateQuorum(uint256 newQuorum) external onlyGovernance {
        ...
        require(newQuorum <= 1000000 * 10**18, "Quorum too high"); // Reasonable upper limit
