...
contract NativeProfileGenesis is Initializable, UUPSUpgradeable {
    ...
    function setJobRating(string memory jobId, address user, uint256 rating) external onlyAuthorized {
        ...
        require(rating > 0 && rating <= 5, "Rating must be 1-5");
        require(jobRatings[jobId][user] == 0, "Rating already submitted");
     ...
     */
    function getProfileAddressesBatch(uint256 startIndex, uint256 count)
        external
        view
