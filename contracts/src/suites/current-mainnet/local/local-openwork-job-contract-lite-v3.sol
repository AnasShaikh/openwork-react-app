...
interface ILayerZeroBridge {
    function sendToNativeChain(string calldata _functionName, bytes calldata _payload, bytes calldata _options)
        external
        payable;
}
    ...
    UUPSUpgradeable
{
    ...
    // v3: Nonzero while applicant milestone selection is awaiting canonical confirmation.
    mapping(string => uint256) public pendingStartApplicationId;
    ...
    event JobStartRequested(string indexed jobId, uint256 indexed applicationId);
    ...
    function initialize(address _owner, address _usdcToken, uint32 _chainId, address _bridge, address _cctpSender)
        public
        initializer
    {
    ...
    function _sendFundsFrom(address _payer, string memory _jobId, uint256 _amount) internal {
        require(cctpSender != address(0), "CCTP not set");

        usdcToken.safeTransferFrom(_payer, address(this), _amount);
        usdcToken.approve(cctpSender, _amount);

        bytes32 mintRecipient = bytes32(uint256(uint160(cctpMintRecipient)));
        // maxFee = 1000 for fast CCTP transfer — NOWJC handles fee tolerance on receive
    ...
    function createProfile(string calldata _ipfsHash, address _referrerAddress, bytes calldata _nativeOptions)
        external
        payable
    {
        bridge.sendToNativeChain{value: msg.value}(
            "createProfile", abi.encode("createProfile", msg.sender, _ipfsHash, _referrerAddress), _nativeOptions
        );
    ...
    function updateProfile(string calldata _newIpfsHash, bytes calldata _nativeOptions) external payable {
        bridge.sendToNativeChain{value: msg.value}(
            "updateProfile", abi.encode("updateProfile", msg.sender, _newIpfsHash), _nativeOptions
        );
    }

    function addPortfolio(string calldata _portfolioHash, bytes calldata _nativeOptions) external payable {
        bridge.sendToNativeChain{value: msg.value}(
            "addPortfolio", abi.encode("addPortfolio", msg.sender, _portfolioHash), _nativeOptions
        );
    }
    ...
    function startDirectContract(
        ...
        _sendFundsFrom(msg.sender, jobId, firstAmount);
    ...
    function startJob(string memory _jobId, uint256 _appId, bool _useAppMilestones, bytes calldata _nativeOptions)
        external
        payable
        nonReentrant
    {
        ...
        require(_appId != 0, "Invalid application");
        require(pendingStartApplicationId[_jobId] == 0, "Start already pending");

        if (_useAppMilestones) {
            pendingStartApplicationId[_jobId] = _appId;
            bytes memory syncPayload = abi.encode("startJobWithMilestoneSync", msg.sender, _jobId, _appId);
            bridge.sendToNativeChain{value: msg.value}("startJobWithMilestoneSync", syncPayload, _nativeOptions);
            emit JobStartRequested(_jobId, _appId);
            return;
        }
        ...
        _sendFundsFrom(msg.sender, _jobId, firstAmount);
        job.currentLockedAmount = firstAmount;
        job.totalEscrowed = firstAmount;
    ...
    /// @notice Complete a pending applicant-milestone start after canonical validation on the native chain.
    /// @dev The job giver must keep sufficient USDC balance and allowance until this retryable callback executes.
    function handleStartJobMilestones(
        ...
        Job storage job = jobs[_jobId];
        require(job.jobGiver == _jobGiver, "Invalid job giver");
        require(job.status == JobStatus.Open, "Not open");
        require(_appId != 0 && pendingStartApplicationId[_jobId] == _appId, "No matching start");
        require(_canonicalAmounts.length > 0, "Must have milestones");

        delete job.milestoneAmounts;
        for (uint256 i = 0; i < _canonicalAmounts.length; i++) {
            require(_canonicalAmounts[i] > 0, "Invalid milestone amount");
            job.milestoneAmounts.push(_canonicalAmounts[i]);
        }

        job.status = JobStatus.InProgress;
        job.currentMilestone = 1;

        uint256 firstAmount = _canonicalAmounts[0];
        _sendFundsFrom(_jobGiver, _jobId, firstAmount);
        job.currentLockedAmount = firstAmount;
        job.totalEscrowed = firstAmount;
        delete pendingStartApplicationId[_jobId];
    ...
    /// @dev No reentrancy guard needed - only external call is bridge.sendToNativeChain
    function submitWork(string calldata _jobId, string calldata _submissionHash, bytes calldata _nativeOptions)
        external
        payable
    {
        bridge.sendToNativeChain{value: msg.value}(
            "submitWork", abi.encode("submitWork", msg.sender, _jobId, _submissionHash), _nativeOptions
        );
    ...
    function lockNextMilestone(string memory _jobId, bytes calldata _nativeOptions) external payable nonReentrant {
        Job storage job = jobs[_jobId];
        require(job.jobGiver == msg.sender, "Not job giver");
        require(job.status == JobStatus.InProgress, "Not in progress");
        ...
        _sendFundsFrom(msg.sender, _jobId, nextAmount);
    ...
    function releaseAndLockNext(string memory _jobId, bytes calldata _nativeOptions) external payable nonReentrant {
        Job storage job = jobs[_jobId];
        require(job.jobGiver == msg.sender, "Not job giver");
        require(job.status == JobStatus.InProgress, "Not in progress");
        require(job.currentLockedAmount > 0, "No funds locked");
    ...
    /// @dev No reentrancy guard needed - only external call is bridge.sendToNativeChain
    function rate(string calldata _jobId, address _userToRate, uint256 _rating, bytes calldata _nativeOptions)
        external
        payable
    {
        bridge.sendToNativeChain{value: msg.value}(
            "rate", abi.encode("rate", msg.sender, _jobId, _userToRate, _rating), _nativeOptions
        );
    }
    ...
    function getEscrowBalance(string memory _jobId)
        external
        view
        returns (uint256 escrowed, uint256 released, uint256 locked)
    {
        Job storage job = jobs[_jobId];
