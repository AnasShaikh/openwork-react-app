...
interface ILocalJobStartReceiver {
...
/// @title LocalLZOpenworkBridge V2
...
contract LocalLZOpenworkBridgeV2 is OAppSender, OAppReceiver {
    ...
    function _lzReceive(
        ...
        // Complete a two-step applicant-milestone job start from the canonical native chain.
        else if (keccak256(bytes(functionName)) == keccak256(bytes("startJobMilestones"))) {
            require(_origin.srcEid == nativeChainEid, "Only native chain");
            _handleStartJobMilestones(_message);
        }
    ...
    function decodeUpgradeMessage(bytes calldata _message)
        external
        pure
        returns (string memory functionName, address targetProxy, address newImplementation)
    {
    ...
    function _handleAthenaMessage(bytes calldata _message) internal {
        ...
        ILocalAthena(athenaClientContract).handleFinalizeDisputeWithVotes(
            disputeId, winningSide, votesFor, votesAgainst, voters, claimAddresses, votingPowers, voteDirections
        );
    }

    function _handleStartJobMilestones(bytes calldata _message) internal {
        require(lowjcContract != address(0), "LOWJC not set");
        (, address jobGiver, string memory jobId, uint256 applicationId, uint256[] memory canonicalAmounts) =
            abi.decode(_message, (string, address, string, uint256, uint256[]));
        ILocalJobStartReceiver(lowjcContract).handleStartJobMilestones(jobGiver, jobId, applicationId, canonicalAmounts);
    }
    ...
    function sendToTwoChains(
        ...
        // Send to main chain
        _lzSend(mainChainEid, _mainChainPayload, _mainChainOptions, fee1, payable(msg.sender));

        // Send to native chain
        _lzSend(nativeChainEid, _nativePayload, _nativeOptions, fee2, payable(msg.sender));
    ...
    /// @param _options LayerZero messaging options
    /// @return fee Native token fee required
    function quoteSpecificChain(uint32 _dstEid, bytes calldata _payload, bytes calldata _options)
        external
        view
        returns (uint256 fee)
    {
    ...
    /// @param _mainChainEid New LayerZero endpoint ID for Main chain
    /// @param _thisLocalChainEid New LayerZero endpoint ID for this chain
    function updateChainEndpoints(uint32 _nativeChainEid, uint32 _mainChainEid, uint32 _thisLocalChainEid)
        external
        onlyOwner
    {
        nativeChainEid = _nativeChainEid;
