...
/**
 * @title XDC CCTP V2 Standard Transceiver — 12 Jul 2026
 * @notice XDC-specific CCTP transceiver using Standard Transfer finality.
 * @dev Dated copy of the deployed transceiver. The legacy sendFast ABI is retained
 *      because LOWJC and LocalAthena already call it, but it deliberately submits
 *      CCTP minFinalityThreshold 2000 (Standard), not 1000 (Fast).
 */
...
contract CCTPTransceiverXdcStandard12Jul2026 {
    ...
    // Events
    event StandardTransferSent(uint256 amount, uint32 destinationDomain, bytes32 mintRecipient, uint256 maxFee);
    ...
    /// @notice ABI-compatible entrypoint used by existing LOWJC and LocalAthena.
    /// @dev The legacy function name is retained, but XDC always uses Standard Transfer finality.
    function sendFast(
        uint256 amount,
        uint32 destinationDomain,
        bytes32 mintRecipient,
        ...
        _sendStandard(amount, destinationDomain, mintRecipient, maxFee);
    }

    /// @notice Explicit Standard Transfer entrypoint for new integrations.
    ...
    function _sendStandard(
        ...
        emit StandardTransferSent(amount, destinationDomain, mintRecipient, maxFee);
    }
