    ...
    UUPSUpgradeable
{
  ...
  /*  function getApplication(string memory _jobId, uint256 _applicationId) public view returns (Application memory) {
    ...
    function releasePaymentCrossChain(
        ...
        // Use actual balance if CCTP fee reduced the received amount (tolerance: 0.01%)
        ...
        if (actualBalance < _amount) {
            ...
            effectiveAmount = actualBalance;
        }

        // Calculate commission on the effective amount
        uint256 commission = calculateCommission(effectiveAmount);
        require(effectiveAmount > commission, "Amount insufficient for commission");
        uint256 netAmount = effectiveAmount - commission;

        // Execute CCTP cross-chain transfer
        _executeCCTPTransfer(_targetRecipient, _targetChainDomain, netAmount);

        // Update state and emit events
        _finalizePayment(_jobGiver, _jobId, job, effectiveAmount, netAmount, commission, _targetRecipient);
    }
    ...
    function _finalizePayment(
        ...
        // Track commission (single point of accumulation)
    ...
    function releasePaymentAndLockNext(address _jobGiver, string memory _jobId, uint256 _releasedAmount, uint256 _lockedAmount) external {
        ...
        // Use actual balance if CCTP fee reduced the received amount (tolerance: 0.01%)
        uint256 actualBalance = usdcToken.balanceOf(address(this));
        uint256 effectiveAmount = _releasedAmount;
        if (actualBalance < _releasedAmount) {
            require(actualBalance >= (_releasedAmount * 9999) / 10000, "Insufficient balance after CCTP fees");
            effectiveAmount = actualBalance;
        }

        // Calculate and deduct commission on effective amount
        uint256 commission = calculateCommission(effectiveAmount);
        uint256 netAmount = effectiveAmount - commission;
