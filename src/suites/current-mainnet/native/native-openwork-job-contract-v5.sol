...
interface IOpenWorkRewards {
    function processJobPayment(address jobGiver, address jobTaker, uint256 amount, uint256 newPlatformTotal)
        external
        returns (uint256[] memory tokensAwarded);
...
interface ICCTPTransceiver {
    function sendFast(uint256 amount, uint32 destinationDomain, bytes32 mintRecipient, uint256 maxFee) external;
...
contract NativeOpenWorkJobContract is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    event JobApplication(
        string indexed jobId, uint256 indexed applicationId, address indexed applicant, string applicationHash
    );
    event JobStarted(
        string indexed jobId,
        uint256 indexed applicationId,
        address indexed selectedApplicant,
        bool useApplicantMilestones
    );
    event WorkSubmitted(string indexed jobId, address indexed applicant, string submissionHash, uint256 milestone);
    event PaymentReleased(
        string indexed jobId, address indexed jobGiver, address indexed applicant, uint256 amount, uint256 milestone
    );
    ...
    event PaymentReleasedAndNextMilestoneLocked(
        string indexed jobId, uint256 releasedAmount, uint256 lockedAmount, uint256 milestone
    );
    ...
    event TokensEarned(
        address indexed user, uint256 tokensEarned, uint256 newPlatformTotal, uint256 newUserTotalTokens
    );
    ...
    function incrementGovernanceAction(address user) external {
        ...
        uint256 currentBand = address(rewardsContract) != address(0) ? rewardsContract.getCurrentBand() : 0;
    ...
    function startJob(address _jobGiver, string memory _jobId, uint256 _applicationId, bool _useApplicantMilestones)
        external
        returns (address selectedApplicant, uint256[] memory canonicalAmounts)
    {
        ...
        require(application.id == _applicationId, "Application ID mismatch");
        require(keccak256(bytes(application.jobId)) == keccak256(bytes(_jobId)), "Application job mismatch");
        ...
        if (_useApplicantMilestones) {
            canonicalAmounts = _setFinalMilestones(_jobId, application.proposedMilestones);
        } else {
            canonicalAmounts = _setFinalMilestones(_jobId, job.milestonePayments);
        }

        emit JobStarted(_jobId, _applicationId, application.applicant, _useApplicantMilestones);
        emit JobStatusChanged(_jobId, IOpenworkGenesis.JobStatus.InProgress);
        return (application.applicant, canonicalAmounts);
    }

    function _setFinalMilestones(string memory _jobId, IOpenworkGenesis.MilestonePayment[] memory _milestones)
        internal
        returns (uint256[] memory canonicalAmounts)
    {
        require(_milestones.length > 0, "Must have milestones");
        canonicalAmounts = new uint256[](_milestones.length);
        for (uint256 i = 0; i < _milestones.length; i++) {
            uint256 amount = _milestones[i].amount;
            require(amount > 0, "Invalid milestone amount");
            canonicalAmounts[i] = amount;
            genesis.addJobFinalMilestone(_jobId, _milestones[i].descriptionHash, amount);
        }
    }
    ...
    function _validateAndCalculatePayment(string memory _jobId, uint256 _amount)
        internal
        view
        returns (IOpenworkGenesis.Job memory job, uint256 netAmount, uint256 commission)
    {
    ...
    function _executeCCTPTransfer(address _recipient, uint32 _targetChainDomain, uint256 _amount) internal {
    ...
    function releasePaymentAndLockNext(
        ...
        if (applicantTargetDomain == 3) {
            ...
            ICCTPTransceiver(cctpTransceiver).sendFast(
                netAmount, applicantTargetDomain, bytes32(uint256(uint160(job.selectedApplicant))), 1000
            );
        }
    ...
    function releasePayment(address _jobGiver, string memory _jobId, uint256 _amount) external {
        ...
        _finalizePayment(_jobGiver, _jobId, job, _amount, netAmount, commission, job.selectedApplicant);
    }
    ...
    function syncRewardsData(bytes calldata _options) external payable {
        ...
        uint256 totalUnlocked =
            address(rewardsContract) != address(0) ? rewardsContract.getUserTotalUnlockedTokens(msg.sender) : 0;

        require(totalUnlocked > 0, "No tokens");

        // Send simple data to bridge
        INativeLZOpenworkBridge(bridge).sendSyncRewardsData{value: msg.value}(msg.sender, totalUnlocked, _options);
    ...
    function releaseDisputedFunds(address _recipient, uint256 _amount, uint32 _targetChainDomain) external {
        ...
        if (_targetChainDomain == 3) {
            ...
            usdcToken.approve(cctpTransceiver, netAmount);
            ICCTPTransceiver(cctpTransceiver).sendFast(
                netAmount, _targetChainDomain, bytes32(uint256(uint160(_recipient))), 1000
            );
        }
