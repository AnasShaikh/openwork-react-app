...
// Canonical job storage is separate from the profile-only Genesis contract.
interface IOpenworkJobGenesis {
    enum JobStatus {
        Open,
        InProgress,
        Completed,
...
contract NativeProfileManager is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    // ==================== STATE VARIABLES ====================
    ...
    // v3: Canonical job storage used to authorize ratings.
    IOpenworkJobGenesis public jobGenesis;

    // Storage gap for upgrade safety (v2 used one slot; v3 consumes one more)
    ...
    event JobGenesisUpdated(address indexed oldJobGenesis, address indexed newJobGenesis);
    ...
    /// @dev Initializes the v3 job-storage dependency atomically with the proxy upgrade.
    function initializeV3(address _jobGenesis) external reinitializer(2) onlyOwner {
        _setJobGenesis(_jobGenesis);
    }
    ...
    function setJobGenesis(address _jobGenesis) external onlyOwner {
        _setJobGenesis(_jobGenesis);
    }

    function _setJobGenesis(address _jobGenesis) internal {
        require(_jobGenesis != address(0), "Invalid address");
        address oldJobGenesis = address(jobGenesis);
        jobGenesis = IOpenworkJobGenesis(_jobGenesis);
        emit JobGenesisUpdated(oldJobGenesis, _jobGenesis);
    }
    ...
    function rate(address _rater, string memory _jobId, address _userToRate, uint256 _rating) external {
        ...
        require(_rater != address(0), "Invalid rater");
        require(_userToRate != address(0), "Invalid user");
        require(_rater != _userToRate, "Cannot rate self");
        require(bytes(_jobId).length != 0, "Invalid job ID");
        require(address(jobGenesis) != address(0), "Job Genesis not set");

        IOpenworkJobGenesis.Job memory job = jobGenesis.getJob(_jobId);
        require(bytes(job.id).length != 0, "Job does not exist");
        require(job.status == IOpenworkJobGenesis.JobStatus.Completed, "Job not completed");
        require(job.selectedApplicant != address(0), "Applicant not selected");

        bool validPair = (_rater == job.jobGiver && _userToRate == job.selectedApplicant)
            || (_rater == job.selectedApplicant && _userToRate == job.jobGiver);
        require(validPair, "Not a job party");
        require(genesis.getJobRating(_jobId, _userToRate) == 0, "Rating already submitted");
    ...
    function getProfile(address _user)
        external
        view
        returns (address userAddress, string memory ipfsHash, address referrerAddress, string[] memory portfolioHashes)
    {
     ...
     */
    function getProfileByIndex(uint256 _index)
        external
        view
        returns (address userAddress, string memory ipfsHash, address referrerAddress, string[] memory portfolioHashes)
    {
