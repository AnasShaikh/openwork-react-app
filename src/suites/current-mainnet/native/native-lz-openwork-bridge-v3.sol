...
interface INativeOpenWorkJobContract {
    ...
    function sendSyncVotingPowerToMainChain(address user, uint256 totalRewards, bytes calldata _options)
        external
        payable;
...
/// @title NativeLZOpenworkBridge V3
...
contract NativeLZOpenworkBridgeV3 is OAppSender, OAppReceiver {
    ...
    function _handleUpgradeMessages(bytes32 fnHash, Origin calldata _origin, bytes calldata _message)
        internal
        returns (bool)
    {
    ...
    function _handleDAOMessages(bytes32 fnHash, bytes calldata _message) internal returns (bool) {
        if (fnHash == keccak256(bytes("updateStakeData"))) {
            revert("Legacy stake sync disabled");
        }
        if (fnHash != keccak256(bytes("updateStakeDataV2"))) return false;

        require(daoStakeSync != address(0), "Stake sync not set");
        (, address staker, uint256 amount, uint256 unlockTime, uint256 durationMinutes, bool isActive, uint64 version) =
            abi.decode(_message, (string, address, uint256, uint256, uint256, bool, uint64));
        INativeDAOStakeSync(daoStakeSync).applyStakeData(staker, amount, unlockTime, durationMinutes, isActive, version);
        return true;
    }
    ...
    function _handleAthenaMessages(bytes32 fnHash, bytes calldata _message) internal returns (bool) {
        ...
        if (fnHash == keccak256(bytes("raiseDispute"))) {
            ...
            INativeAthena(nativeAthenaContract).handleRaiseDispute(
                jobId, disputeHash, oracleName, fee, disputedAmount, disputeRaiser
            );
            return true;
        }
        ...
        if (fnHash == keccak256(bytes("submitSkillVerification"))) {
            ...
            INativeAthena(nativeAthenaContract).handleSubmitSkillVerification(
                applicant, applicationHash, feeAmount, targetOracleName
            );
            return true;
        }
    ...
    function _handleProfileMessages(bytes32 fnHash, bytes calldata _message) internal returns (bool) {
        ...
        if (fnHash == keccak256(bytes("createProfile"))) {
            ...
            (, address user, string memory ipfsHash, address referrer) =
                abi.decode(_message, (string, address, string, address));
        ...
        if (fnHash == keccak256(bytes("updatePortfolioItem"))) {
            require(profileManager != address(0), "ProfileManager not set");
            (, address user, uint256 index, string memory newPortfolioHash) =
                abi.decode(_message, (string, address, uint256, string));
        ...
        returns (bool)
    {
        ...
        if (fnHash == keccak256(bytes("postJob"))) {
            ...
            INativeOpenWorkJobContract(nativeOpenWorkJobContract).postJob(
                jobId, jobGiver, jobDetailHash, descriptions, amounts
            );
            return true;
        }
        ...
        if (fnHash == keccak256(bytes("applyToJob"))) {
            ...
            INativeOpenWorkJobContract(nativeOpenWorkJobContract).applyToJob(
                applicant, jobId, applicationHash, descriptions, amounts, preferredChainDomain
            );
            return true;
        }
        ...
        if (fnHash == keccak256(bytes("releasePaymentAndLockNext"))) {
            ...
            INativeOpenWorkJobContract(nativeOpenWorkJobContract).releasePaymentAndLockNext(
                jobGiver, jobId, releasedAmount, lockedAmount
            );
            return true;
        }
        ...
        if (fnHash == keccak256(bytes("releasePaymentCrossChain"))) {
            ...
            INativeOpenWorkJobContract(nativeOpenWorkJobContract).handleReleasePaymentCrossChain(
                jobGiver, jobId, amount, targetChainDomain, targetRecipient
            );
            return true;
        }
        ...
        if (fnHash == keccak256(bytes("startDirectContract"))) {
            ...
            INativeOpenWorkJobContract(nativeOpenWorkJobContract).handleStartDirectContract(
                jobGiver, jobTaker, jobId, jobDetailHash, descriptions, amounts, jobTakerChainDomain
            );
            return true;
        }
    ...
    function sendToMainChain(string memory _functionName, bytes memory _payload, bytes calldata _options)
        external
        payable
        onlyAuthorized
    {
        _lzSend(mainChainEid, _payload, _options, MessagingFee(msg.value, 0), payable(msg.sender));
    ...
    function sendToLocalChain(
        ...
        _lzSend(targetEid, _payload, _options, MessagingFee(msg.value, 0), payable(msg.sender));
    ...
    /// @param _options LayerZero messaging options
    function sendSyncRewardsData(address user, uint256 claimableAmount, bytes calldata _options)
        external
        payable
        onlyAuthorized
    {
        bytes memory payload = abi.encode("syncClaimableRewards", user, claimableAmount);
    ...
    function sendSyncVotingPower(address user, uint256 totalRewards, bytes calldata _options)
        external
        payable
        onlyAuthorized
    {
        bytes memory payload = abi.encode("syncVotingPower", user, totalRewards);
    ...
    function quoteSyncVotingPower(address user, uint256 totalRewards, bytes calldata _options)
        external
        view
        returns (uint256 fee)
    {
    ...
    /// @param _options LayerZero messaging options
    /// @return fee Native token fee required
    function quoteSyncRewardsData(address user, uint256 claimableAmount, bytes calldata _options)
        external
        view
        returns (uint256 fee)
    {
    ...
    /// @param _options LayerZero messaging options
    /// @return fee Native token fee required
    function quoteLocalChain(string memory _disputeId, bytes calldata _payload, bytes calldata _options)
        external
        view
        returns (uint256 fee)
    {
    ...
    function setDAOStakeSync(address _daoStakeSync) external onlyOwner {
        require(_daoStakeSync != address(0), "Invalid stake sync");
        address oldContract = daoStakeSync;
        daoStakeSync = _daoStakeSync;
        emit DAOStakeSyncUpdated(oldContract, _daoStakeSync);
    }
